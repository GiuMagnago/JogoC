...
int Personagens::pocaoVida() {
    ...
    if (pontosV == vidaMax) //se a vida já estiver no máximo, não haverá cura
    {
        return 0;
    }
    else if (vidaMax < (pontosV + cura)) //checa se a vida atual somada com a cura ficará maior do que a vida máxima, caso fique, é curada somente até a vida máxima
    {
        cura = vidaMax - pontosV;
    }
...
void Personagens::recuperaMana(int qntMana) {
    attMana(qntMana);
}

void Personagens::attMana(int att) {
    int manaMax = pontosM;

    if (manaMax < (pontosM + att))
    {
        pontosM += manaMax - pontosM;
    }
}
